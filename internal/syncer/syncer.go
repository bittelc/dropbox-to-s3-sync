...
func (s *Syncer) RunOnce(ctx context.Context, DropboxPath string) error {
	log.Printf("Sync pass started for DropboxPath: %s", DropboxPath)
	dropBoxFiles, err := s.Dropbox.ListFiles(ctx, DropboxPath)
	if err != nil {
		return fmt.Errorf("list dropbox files: %w", err)
	}
	// Build set of desired keys
	desiredKeys := make(map[string]dropbox.File, len(dropBoxFiles))
